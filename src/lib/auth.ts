...
export async function requireAuth(req: NextRequest): Promise<AuthResult> {
  ...
  if (!stored) return { ok: false, error: 'Token has been revoked' };
...
export async function requireAdmin(req: NextRequest) {
  ...
  if (!auth.user.is_admin) return { ok: false as const, error: 'Admin access required' };

  const admin = await prisma.admin.findUnique({ where: { user_id: auth.user.sub } });
  if (!admin) return { ok: false as const, error: 'Admin record not found' };
