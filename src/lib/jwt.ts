...
export async function verifyAccessToken(token: string): Promise<VerifiedPayload> {
  ...
  return {
    ...
    manageGroups: (payload['manageGroups'] as boolean) ?? false,
    managePetitions: (payload['managePetitions'] as boolean) ?? false,
    manageFaq: (payload['manageFaq'] as boolean) ?? false,
