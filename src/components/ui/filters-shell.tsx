...
// ─── Section title (used by panel content) ────────────────────────────────────
...
// ─── Public trigger button + sheet/popover dispatcher ─────────────────────────
...
export function FiltersButton({ activeCount, className, children }: FiltersButtonProps) {
  const [open, setOpen] = useState(false);
  ...
  return (
    <div className={cn('relative', className)}>
      ...
      {isMobile ? (
        <MobileFiltersSheet open={open} onClose={() => setOpen(false)}>
          {children}
        </MobileFiltersSheet>
      ) : (
        <DesktopFiltersPopover open={open} onClose={() => setOpen(false)}>
          {children}
...
// ─── Reset filters button (shared bottom of panel content) ────────────────────
