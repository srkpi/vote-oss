...
export const buildMedianMetric: MetricBuilder = ({ metrics, totalBallots }) => {
  ...
  if (totalBallots === 2) {
    ...
    insight =
      `При двох бюлетенях медіанна позначка (${mp.toFixed(0)}%) вказує лише на те, де в часі розташовано середнє між двома голосами. ` +
      `Статистичне значення цього показника зростає зі збільшенням кількості учасників.`;
  } else if (isExtremeEarly) {
    insight =
      `Медіанний бюлетень надійшов на позначці ${mp.toFixed(0)}% від загальної тривалості — ` +
      `надзвичайно ранній момент. Понад половина учасників проголосувала ще до того, ` +
      `як минула шоста частина відведеного часу. ` +
    ...
    insight =
      `Половина голосів зібрана вже на ${mp.toFixed(0)}%-й позначці тривалості. ` +
      `Перша третина часового вікна несла непропорційно більше навантаження, ніж друга. ` +
    ...
    if (mp >= 45 && mp <= 55) {
      insight =
        ...
        `Перша і друга половини за кількістю бюлетенів практично рівні. ` +
      ...
      insight =
        `Медіана ${mp.toFixed(0)}% — незначний ухил до першої половини. ` +
      ...
      insight =
        `Медіана ${mp.toFixed(0)}% — незначне зміщення до другої половини. ` +
    ...
    insight =
      ...
      `це означає, що більшу частину тривалості голосування було майже порожнім. ` +
    ...
    insight =
      `Медіана зміщена до пізнішої частини: половина голосів зібрана після ${mp.toFixed(0)}% тривалості. ` +
